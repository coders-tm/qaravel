        ...
        <div class="q-gutter-md">
          ...
          >
            ...
            <div class="col-xs-12 col-sm-4">
              ...
              <q-input dense outlined v-model="staff.surname" type="text" />
            </div>
            <div class="col-xs-12 col-sm-4">
              ...
              <q-input dense outlined v-model="staff.calendar_color">
                ...
                <template v-slot:append>
                  <q-icon name="colorize" class="cursor-pointer">
                    ...
                    >
                      <q-color v-model="staff.calendar_color" format-model="hex" />
                    </q-popup-proxy>
                  </q-icon>
                </template>
              </q-input>
            ...
            <div class="col-xs-12 col-sm-4">
              <div class="text-label">Job Role</div>
              <q-input dense outlined v-model="staff.job_role" type="text" />
            </div>
          </base-section>
          <base-section title="Security" description="Security to make your accounts unbreakable.">
            <div class="col-xs-12 col-sm-4">
            ...
            <div class="col-xs-12">
              ...
              <div class="row q-col-gutter-md">
                <div class="col-xs-12 col-sm-4" v-for="(item, index) in modules" :key="item.id">
                  <permissions
                    v-model:group-permissions="staff.groupPermissions"
                    v-model:permissions="staff.permissions"
                    :edit="edit"
...
export default {
  ...
  methods: {
    ...mapActions('staff', ['View', 'Store', 'Update']),
    ...mapActions('app', ['Modules']),
    onSubmit(props) {
      console.func('pages/core/staffs/staff:methods.onSubmit()', arguments);
      this.submited = true;
      if (this.creating) {
        this.Store(this.staff)
          .then(({ data, message }) => {
            this.submited = false;
    ...
    onReset(props) {
      console.func('pages/core/staffs/staff:methods.onReset()', arguments);
      this.loaded = false;
      setTimeout(() => {
        Object.assign(this.staff, cloneDeep(this.default));
        this.loaded = true;
      }, 1);
    },
    onCancel(props) {
      console.func('pages/core/staffs/staff:methods.onCancel()', arguments);
      this.$router.go(-1);
    },
    onChangeGroup(groups) {
      console.func('pages/Staffs:methods.onChangeGroup()', arguments);
      if (groups.length < 1) {
      ...
      groups.forEach((group) => {
        if (group.permissions) {
          group.permissions.forEach((permission) => {
            this.staff.groupPermissions.push(permission.id);
          });
        }
      });
    },
  ...
  computed: {
    ...
    disable() {
      return this.default && JSON.stringify(this.staff) === JSON.stringify(this.default);
    },
    resetable() {
      return this.default && JSON.stringify(this.staff) !== JSON.stringify(this.default);
    },
