...
import { useFinanceTypeStore } from "stores/finance-type";
...
export default {
  ...
  methods: {
    ...mapActions(useFinanceTypeStore, ["get"]),
    onRequest(props) {
      console.func("admins/members/TypePage:methods.onRequest()", arguments);
    ...
    rowClicked(evt, row) {
      console.func(
        "pages/admins/finance/TypePage:methods.rowClicked()",
        arguments
      );
      this.$q.dialog({
  ...
  computed: {
    ...mapState(useFinanceTypeStore, ["columns", "toolbar", "rows"]),
    permissions() {
      return [];
    },
  },
